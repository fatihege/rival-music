...
export default function ChangeProfileModal() {
    ...
    const getUserProfileColors = async () => {
        ...
        const userData = await getUserData(user.id, 'profileColor,accentColor') // Get profile color and accent color of the active user
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!user.profileColor || user.profileColor?.toString() !== profileColorRef.current?.toString()) // If the user is not has a profile color or the new profile color value is different from the user's profile color
