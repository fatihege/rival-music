...
export default function PlaylistContextMenu({playlist}) {
    ...
    const [isLiked, setIsLiked] = useState(null) // Is track liked
    ...
    return user?.loaded && user?.id && user?.token ? (
        <div className={styles.contextMenu} ref={menuRef} style={{
            ...
            <div className={`${styles.item} ${!playlist?.tracks?.filter(t => !!t.audio)?.length ? styles.disabled : ''}`} onClick={handleAddToQueue}>
