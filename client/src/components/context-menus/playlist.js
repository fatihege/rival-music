...
export default function PlaylistContextMenu({playlist}) {
    ...
    const handleCopyLink = () => {
        if (!playlist?.id && !playlist?._id) return // If navigator clipboard is not exist or playlist is not exist, return
        if (navigator?.clipboard) navigator.clipboard.writeText(`${process.env.APP_URL}/playlist/${playlist?.id || playlist?._id}`) // Copy link to clipboard
        else if (document) {
            const el = document.createElement('textarea') // Create textarea
            el.value = `${process.env.APP_URL}/playlist/${playlist?.id || playlist?._id}` // Set textarea value
