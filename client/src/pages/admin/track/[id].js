...
export default function EditTrackPage({id}) {
    ...
    return user.loaded && user?.admin ? (
        <>
            <Head>
                <title>Edit Track{track?.title ? `: ${track?.title?.toString()}` : ''} — Rival Music</title>
            </Head>
            <div className={styles.mainContainer}>
                ...
                <div className={styles.form}>
                    ...
                    <div>
                                    ...
                                    return (
                                        <Link href={'#'} key={i} className={styles.album} onClick={e => {
                                            ...
                                            <div className={styles.info}>
                                                ...
                                                <div className={styles.artistName}>{album?.artist?.name}</div>
                                            </div>
                                        </Link>
                                    )
                                })}
                        ...
                        {album && album?.title ? (
                            <div className={styles.album}>
                                <div className={styles.image}>
                                    {album?.cover ?
                                        <img src={`${process.env.IMAGE_CDN}/${album?.cover}`} alt={album?.title}/> :
                                        <AlbumDefault/>}
                                </div>
                                <div className={styles.info}>
                                    <div className={styles.title}>{album?.title}</div>
                    ...
                    <div>
                        <h3 className={styles.formTitle}>Artists</h3>
                        <Input placeholder="Search for foreign artists" onChange={value => setArtistQuery(value)}
                                            ...
                                            <div className={styles.image}>
                                                {artist?.image ?
                                                    <img src={`${process.env.IMAGE_CDN}/${artist?.image}`}
                                                         alt={artist?.name}/> : artist?.name?.[0].toUpperCase()
                                                }
                                            </div>
                                            <div className={styles.info}>
                        ...
                        {artists?.length ? artists.map((artist, i) => (
                            <div className={styles.artist} key={i}>
                                ...
                                <button className={styles.remove} onClick={() => {
                                    setTrack({
                                        ...track,
                                        artists: track?.artists?.filter(a => a !== artist?._id),
                                    })
                                    setArtists([
                                        ...artists.slice(0, i),
                                        ...artists.slice(i + 1),
