...
export default function ViewAlbumsPage() {
    ...
    const getAlbums = async (fromZero = false) => {
        ...
        try {
            const response = await axios.get(`${process.env.API_URL}/album?cursor=${cursorRef.current}&limit=${LIMIT}&sorting=${sortingRef.current}${search?.trim()?.length ? `&query=${search?.trim()}` : ''}&populate=artist`) // Get albums from API
